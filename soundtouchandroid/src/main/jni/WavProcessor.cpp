...
void WavProcessor::processWav(std::string src, float tempoDelta, float pitchDelta, float rateDelta, std::string dst)
{
    char *args[3] = {NULL, (char *)src.c_str(), (char *)dst.c_str()};

    RunParameters *params = new RunParameters(0, args);

    params->inFileName = (char *)src.c_str();
    params->outFileName = (char *)dst.c_str();
